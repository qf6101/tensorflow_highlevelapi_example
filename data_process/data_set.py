...
def _compose_dense_feature(label, f_indices, f_values, feature_size, label_size, is_binary_model):
    if not is_binary_model:
        label = tf.one_hot(indices=tf.cast(label, tf.int64), depth=label_size)
    return f_values, label
...
def read_dataset(config):
    ...
    if config.input_is_sparse:
        compose_feature_fn = _compose_sparse_feature
    else:
        compose_feature_fn = _compose_dense_feature

    tensor_dataset = records_dataset.map(
        functools.partial(compose_feature_fn, feature_size=config.feature_size, label_size=config.label_size,
